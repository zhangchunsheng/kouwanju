if(window.IDE) {
    IDE.ComponentRegistry.register('mainUI.component.bagPager', [{
        name : 'pageNumber',
        type : 'IDE_number',
        required: true,
        value : 0
...
wozllajs.defineComponent('mainUI.component.bagPager', {

    extend : 'Behaviour',

    alias : 'mainUI.bagPager',
    ...
    onTouchEnd : function(e){
        ...
        if(this.startX - this.endX > this.judgementX){
            ...
            this.restorePage();
            if(Math.abs(this.endX - this.startX)<this.overlookX){
                var width = 125;
                var height = 95;
                var x = [];
                var y = [];
                ...
                y[2] = 285;
                y[3] = 380;
                mark: for(var i = 0; i < 3; i++){
                    for(var j = 0; j < 4; j++){
                        if(tx>x[i] && tx<x[i]+width){
                            if(ty>y[j] && ty<y[j]+height){
                                this.notify('BI.US');
                                ...
                                this.notify('BI.P'+this.pageNo+num);
                                this.IUC = true;
    ...
    update : function(){
        ...
        this.IUC && this.gameObject.updateCache(0, 0);
        this.fromX = newX;
    },
