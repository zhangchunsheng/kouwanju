...
(function($) {
    ...
    Triangle.prototype = {
        ...
        isPointIn: function(testPoint) {
            ...
            var interiorCount = 0;
            for (var i = 0; i < 3; i++) {
                flag = this.sides[i].classifyPoint(testPoint, EPSILON);
                if (flag != $.Vector2f.LEFT_SIDE) {
                    interiorCount++;
                }
            }
...
(function($) {
    ...
    NavMesh.prototype = {
        ...
        findPath: function(startPointPx, endPointPx) {
            ...
            var endCell = this.getCellByPoint(endPos);
            if(!endCell) {
                return null;
            }
            var startCell = this.getCellByPoint(startPos);
            if (!startCell) {
                return null;
            }
            var outPath;
